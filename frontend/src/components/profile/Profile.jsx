...
function UserInfoCard({ userInfo, publicAddress, accountBalance, getBalance }) {
    ...
    return (
        <Card style={{ width: '20rem' }} className="mb-3">
            <Card.Img variant="top" src={userInfo?.profileImage ?? DEFAULT_PROFILE} alt={`${userInfo?.name}'s profile`} />
            <Card.Body>
                <Card.Title>{userInfo.name}</Card.Title>
                <Card.Subtitle className="mb-2 text-muted">{userInfo?.email}</Card.Subtitle>
                {userInfo?.typeOfLogin && <Card.Text>
                    <strong>Type of Login:</strong> {userInfo?.typeOfLogin}
                </Card.Text>}
                {userInfo?.verifierId && <Card.Text>
...
function Profile() {
    const { login, loggedIn, logout, userInfo, accountBalance, publicAddress, getUserInfo, getAccounts, getBalance } = useAuth();
    const loggedInView = (
        <>
            <UserInfoCard userInfo={userInfo} accountBalance={accountBalance} publicAddress={publicAddress} getBalance={getBalance} />
        </>
    );
